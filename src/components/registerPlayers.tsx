...
function DeleteButton(props: { onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      className="text-white bg-red-500 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800"
      onClick={onClick}
    >
      <svg
        fill="#FFFFFF"
...
function Player(props: {
  ...
  return (
    <tr>
      <td width="250px">{players[index]}</td>
      <td>
...
export default function RegisterPlayers(props: {
  ...
  function addPlayer(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (
      !players.includes(playerToBeAdded) &&
      playerToBeAdded !== "" &&
      playerToBeAdded.length <= maxPlayerLength
    ) {
      // Add player
